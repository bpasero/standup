	...
	function render(stage) {
		...
		stageList = stageList.concat(stage.order.map(function(actor, index) {
			if (index === stage.current) {
				...
				return format('<span class="list-group-item list-group-item{0}" style="border-top-left-radius: 0; border-top-right-radius: 0;"><h2><a style="color: #5a5a5a;" href="{1}">{2}</a></h2></span>', className, actor.name.toLowerCase() === 'redmond' ? redmondStatus : zurichStatus, actor.name);
			}
